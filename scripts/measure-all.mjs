...
function updateReadme(runs) {
	...
	const block = [
		...
		'',
		'Methodology: `1 - (mismatched pixels / total pixels)` via [`pixelmatch`](https://github.com/mapbox/pixelmatch), scored against a Word PDF export (not a Word editing-view screenshot, so the diff reflects rendered output, not non-printing marks). `docx-preview-sync` is measured through its async path, the highest-scoring path for this fixture. Reproduce with `pnpm measure:all`; on WSL/Linux it uses a project-local fontconfig file and includes `/mnt/c/Windows/Fonts` when present, so `agent-browser` measures with Windows Japanese fonts instead of fallback fonts.',
	].join('\n');
