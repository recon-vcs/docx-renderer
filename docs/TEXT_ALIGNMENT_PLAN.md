...
### 重要な訂正: 「途中からズレる」のではなく「段落ごとに独立してズレている」

表題はページ1個目の段落で、直前に積み上がるものが無い。にもかかわらず表題自身の ink 位置から既に reference とズレている（表題の ink top: reference 158px / rendered 145px、共に section 先頭からの offset）。これは「行を重ねるうちに誤差が蓄積する」モデルでは説明できない。

切り分けのため、ページ先頭の固定ジオメトリ（header 高さ、header 下端から本文開始までの余白）を p1・p2 で比較したところ両方とも 37.625px で完全一致。p2 はスコアも高い。つまりページ土台（余白・ヘッダー）は正常で、ここが原因ではない。

残る説明は「表題という段落単体の line-height が不足している」こと自体が、(a) 表題の ink 位置と (b) 表題→内容の gap の両方に同時に効いているというもの。line-height の余剰（もしくは不足）はボックス内で上下均等に配分されるため、不足分を ΔLH とすると ink のずれは ΔLH/2（実測13px → ΔLH≈26）、次段落への gap 影響は ΔLH 丸ごと（実測-20px 前後）で、桁も方向も整合する。

結論: 表題からズレて見えるのも TOC・見出し行のズレと同じ「line-height 不足」由来。ただし累積ではなく、**各段落が個別に単独で高さを間違えており、たまたま最初に来る段落（表題）でそれが可視化されているだけ**。「途中から崩れる」という見え方はしない。
